...
- **新增** 三端联动自动登出：MTWS 与 OMICS 前端登录后启动后台轮询（每 30 秒）`/auth/status`；一旦发现统一登录态被控制台清空（过期），立即自动登出本页面 UI：MTWS 显示「登录过期」错误页并停止自动刷新，OMICS 弹窗提示并回到未登录状态，避免“显示已登录但实际报 API 失败”。
- **说明** 处理了“token 在别的电脑登录或长时间未请求数据后失效”的场景：以前这种情况下本地仍保存 token、显示已登录，但业务请求会报 API 失败；现在由控制台主动探测失效并驱动三端登出。
- **验证** `launcher.py` 通过 `py_compile`（无 SyntaxWarning）；`MTWS/main.js`、`OMICS/script.js` 通过 `node --check`；重启启动器实测：推送 token 后 `/auth/status` 返回 `expired` 字段；在 SF 内网不可达的开发机上校验周期不会误登出（网络错误保留登录态）。

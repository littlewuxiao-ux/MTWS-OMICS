...
%PY% -m pip install flask waitress pandas openpyxl requests python-dateutil customtkinter
if errorlevel 1 (echo 依赖安装失败，请检查网络或代理设置。 & pause & exit /b 1)
echo [3/3] 验证项目导入...
set "PYTHONPATH=%CD%;%CD%\backend;%PYTHONPATH%"
%PY% -c "from backend.logic.exporter import process_stats_and_save; import flask,pandas,openpyxl,requests,customtkinter; print('OMICS 环境正常')"
